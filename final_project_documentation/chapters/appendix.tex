...
Every ingress and egress rule attaches at the resource level through a Network
Security Group rather than a subnet-wide security list. Tables~\ref{tab:nsg-api}
through~\ref{tab:nsg-lb} list the rules declared in
\texttt{modules/network/locals.tf} for each of the four subnets
...
The rules marked \emph{optional, enabled} come from boolean switches in
\texttt{modules/network/variables.tf} (SSH access, general worker outbound, pod
...
\begin{table}[H]
\centering
\caption{Commands used to deploy, operate, and tear down the system, from \texttt{Terraform-code/DEPLOYMENT.md}.}
